...
class Overdrag extends EventEmitter {
    static __ENGAGED_STACK__ = [];
    static ERROR = {
    ...
    element;
    parentElement;
    noBoundsCache = { top: 0, right: 0, bottom: 0, left: 0 };
    snapThreshold;
    controlsThreshold;
    minContentHeight;
    minContentWidth;
    ...
    downMouseX = 0;
    downMouseY = 0;
    constructor({ element, minContentHeight = Overdrag.DEFAULTS.minContentHeight, minContentWidth = Overdrag.DEFAULTS.minContentWidth, maxContentWidth = Overdrag.DEFAULTS.maxContentWidth, maxContentHeight = Overdrag.DEFAULTS.maxContentHeight, snapThreshold = Overdrag.DEFAULTS.snapThreshold, controlsThreshold = Overdrag.DEFAULTS.controlsThreshold, clickDetectionThreshold = Overdrag.DEFAULTS.clickDetectionThreshold, stack = Overdrag.DEFAULTS.stack, excludePadding = Overdrag.DEFAULTS.excludePadding, }) {
        super();
    ...
    assignInitialStyles() {
        ...
        this.element.style.right = `${this.parentPosition.actionBounds.right - this.position.fullBounds.right}px`;
        this.element.style.bottom = `${this.parentPosition.actionBounds.bottom - this.position.fullBounds.bottom}px`;
    ...
    isClick() {
        return (Math.abs(this.parentMouseX - this.downMouseX) <
    ...
    updateControlPointsState() {
        ...
        const inYBounds = this.parentMouseY >= this.position.visualBounds.top &&
            this.parentMouseY <= this.position.visualBounds.bottom;
        const inXBounds = this.parentMouseX >= this.position.visualBounds.left &&
        ...
        const controlsActive = this.controls.left ||
            this.controls.right ||
            this.controls.top ||
            this.controls.bottom;
        if (controlsActive && current !== JSON.stringify(this.controls)) {
            this.controlsActive = true;
            this.element.setAttribute(Overdrag.ATTRIBUTES.CONTROLS, Object.keys(this.controls)
    ...
    updateCursorStyle() {
        ...
        else if (Overdrag.CURSOR.RIGHT_BOTTOM &&
            this.controls.bottom &&
            this.controls.right) {
            cursor = Overdrag.CURSOR.RIGHT_BOTTOM;
        }
        else if (Overdrag.CURSOR.LEFT_BOTTOM &&
            this.controls.bottom &&
            this.controls.left) {
            cursor = Overdrag.CURSOR.LEFT_BOTTOM;
        }
        else if (Overdrag.CURSOR.RIGHT_TOP &&
            this.controls.top &&
            this.controls.right) {
    ...
    movePointRight() {
        const width = Math.max(this.minContentWidth, Math.min(this.maxContentWidth, this.calcRight() -
    ...
    movePointBottom() {
        const height = Math.max(this.minContentHeight, Math.min(this.maxContentHeight, this.calcBottom() -
    ...
    movePointLeft() {
        ...
        const left = Math.min(Math.max(this.calcLeft(), // this will track mouse within action bounds of the parent
        ...
        const width = this.downPosition.fullBounds.right - left - this.position.horizontalDiff;
    ...
    movePointTop() {
        ...
        const top = Math.min(Math.max(this.calcTop(), // this will track mouse within action bounds of the parent
        ...
        const height = this.downPosition.fullBounds.bottom - top - this.position.verticalDiff;
    ...
    calcRight() {
        let right = this.parentMouseX - this.offsetX + this.downPosition.fullBounds.width;
    ...
    calcBottom() {
        let bottom = this.parentMouseY - this.offsetY + this.downPosition.fullBounds.height;
        // snap to the edges of the window
        bottom =
    ...
    drag() {
        ...
        x + this.position.fullBounds.width + this.snapThreshold >=
            ...
            : // otherwise, use the current mouse position
                x;
        if (this.position.fullBounds.left !== left ||
